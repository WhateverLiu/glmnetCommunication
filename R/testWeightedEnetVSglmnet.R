...
system.time({glmnetRst = testWeightedEnet(
  Xr = Xtrain, yr = ytrain, Wtrain = rep(1, Nrow),
  XrVali = Xvalid, yrVali = yvalid, Wvalid = rep(1, nrow(Xvalid)),
  alpha = 1.0, lambdaMinMaxRatio = 0.01, lambdaPathLength = 100,
  maxIter = 100, earlyStoppingRounds = 100, eps = 1e-10,
  lambdaPathStopDeviance = 0.999,
  lambdaPathStopDevianceRelaIncrease = 1e-5)})
